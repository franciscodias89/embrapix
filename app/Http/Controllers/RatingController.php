        ...
        if ($user) {
            ...
            if ($order) {
               ...
               if($request->restaurant_rating){
                ...
                $ratings = Rating::where('order_id', $order->id)->where('rateable_type','App\Restaurant')->first();
                if ($ratings) {
                    Rating::where('order_id', $order->id)->where('rateable_type','App\Restaurant')
                    ->update(['rating' => $request->restaurant_rating, 'comment' => $request->comment]);
                } else {
            ...
            if($request->delivery_rating){
                ...
                if($deliveryGuy){
                    ...
                    $ratings = Rating::where('order_id', $order->id)->where('rateable_type','App\User')->first();
                if ($ratings) {
                    Rating::where('order_id', $order->id)->where('rateable_type','App\User')
                    ->update(['rating' => $request->delivery_rating, 'comment' => $request->comment]);
                }else{
