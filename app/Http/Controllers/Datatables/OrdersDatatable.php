...
class OrdersDatatable
{
    public function ordersDataTable()
    {
        ...
        return Datatables::of($orders)
            ->editColumn('unique_order_id', function ($order) {
                ...
                $html .= '<a href="' . route('panel.viewOrder', $order->unique_order_id) . '"target="_blank" class="linked-item" data-popup="tooltip" title="" data-placement="bottom" data-original-title="Open order in a new tab">' . $order->unique_order_id . '</a>';
            ...
            ->editColumn('total', function ($order) {
                return 'R$ ' . number_format($order->total, 2, ',', '.');
            })
            ...
            ->addColumn('action', function ($order) {
                $html = '<a href="' . route('panel.viewOrder', $order->unique_order_id) . '" class="btn btn-sm btn-primary"> View</a>';
                return $html;
            })
    ...
    public function ordersDataTableRestaurant()
    {
        // sleep(5000);
        $user = Auth::user();
        $restaurant_id=$user->restaurant_id;
        $orders = Order::with('orderstatus', 'accept_delivery.user', 'restaurant')->where('restaurant_id',$restaurant_id);
        ...
        return Datatables::of($orders)
            ...
            ->editColumn('created_at', function ($order) {
                return '<span data-popup="tooltip" data-placement="left" title="' . $order->created_at->diffForHumans() . '">' . $order->created_at->format('d-m-Y - h:i') . '</span>';
            })
            ->editColumn('orderstatus_id', function ($order) {
                ...
                $html .= '<br> <small> Pedido por: ' . $order->user->name . '</small>';

                if ($order->orderstatus_id > 2 && $order->orderstatus_id < 6) {
                    if ($order->accept_delivery && $order->accept_delivery->user && $order->accept_delivery->user->name) {
                        $html .= '<br> <small> Entregue por: ' . '<a href="' . route('admin.get.editUser', $order->accept_delivery->user->id) . '"target="_blank" class="linked-item">' . $order->accept_delivery->user->name . '</a></small>';
                    }
                } else {
                    $html .= '</div>';
                }
            ...
            ->addColumn('action', function ($order) {
                $html = '<a href="' . route('panel.viewOrder', $order->unique_order_id) . '" class="btn btn-sm btn-primary"> Ver Pedido</a>';
                return $html;
            })
            ->addColumn('live_timer', function ($order) {
                $html = '';
                if ($order->orderstatus_id == 5) {
                    ...
                    $html .= '<p class="order-dashboard-time text-center mix-fit-content mt-1"><b>Entregue em:<br> </b><small>' . timeStrampDiffFormatted($order->created_at, $order->updated_at) . '</small></p>';
                } elseif ($order->orderstatus_id == 6) {

                    $html .= '<p class="order-dashboard-time text-center mix-fit-content mt-1"><b>Cancelado em:<br> </b><small>' . timeStrampDiffFormatted($order->created_at, $order->updated_at) . '</small></p>';
                } else {

                    $html .= '<div class="text-center"><strong>Tempo: </strong> <small><p class="liveTimer mt-1 text-center mix-fit-content order-dashboard-time" title="' . $order->created_at . '"><i class="icon-spinner10 spinner position-left"></i></p> </small></div>';
                }
                return $html;
            })
            ->rawColumns(['unique_order_id', 'orderstatus_id', 'action', 'created_at', 'live_timer'])
