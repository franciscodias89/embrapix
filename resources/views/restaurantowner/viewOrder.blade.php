                ...
                <div class="col-md-12">
                    <div class="d-flex justify-content-end">
                        ...
                        <a href="{{ route('restaurant.acceptOrder', $order->id) }}"
                            class="btn btn-primary"> {{__('storeDashboard.ovAccept')}} <i
                            class="icon-checkmark3 ml-1"></i></a></div>
                            <div class="btn-group btn-group-justified" style="width: 150px">  
                        <a href="{{ route('restaurant.cancelOrder', $order->id) }}"
                            class="btn btn-danger" data-popup="tooltip" data-placement="top" title="{{__('storeDashboard.ovTooltipCancel')}}"> {{__('storeDashboard.ovCancel')}} <i
                            class="icon-cross ml-1"></i></a></div>
                        @endif
                        @if($order->delivery_type == 2 && $order->orderstatus_id == 2)
                        <div class="btn-group btn-group-justified" style="width: 150px">
                        <a href="{{ route('restaurant.markOrderReady', $order->id) }}"
                            class="btn btn-warning"> {{__('storeDashboard.ovMarkAsReady')}} <i
                            class="icon-checkmark3 ml-1"></i></a></div>
                        @endif 
                        @if($order->delivery_type == 2 && $order->orderstatus_id == 7)
                        
                        <a href="{{ route('restaurant.markSelfPickupOrderAsCompleted', $order->id) }}"
                            class="btn btn-success"> {{__('storeDashboard.ovMarkAsCompleted')}} <i
